...
package object api {
    ...
    object Formats {
    	...
    	/*
    	implicit object ImageFormat extends PartialFormat[DockerImage] {
		    def partialReads: PartialFunction[JsValue, JsResult[DockerImage]] = {
		      case JsObject(
		          Seq(
		              ("Id", JsString(id)), 
		              ("ParentId", JsString(pId)),
		              ("RepoTags", JsArray(tags)),
		              ("Created", JsNumber(created)),
		              ("Size", JsNumber(size)),
		              ("VirtualSize", JsNumber(vSize)),
		              ::)) => JsSuccess(DockerImage(id, Some(pId), Seq.empty, new org.joda.time.DateTime(created.toLongExact), size.toLong, vSize.toLong))
		      
		    }
		    
		    val partialWrites: PartialFunction[DockerEntity, JsValue] = {
		      case image: DockerImage => Json.obj("Id" -> JsString(image.Id))
		    }
    	}
    	*/
    ...
    implicit val dockerImageSearchResultFmt = Format(
        (
            ...
            (__ \ "star_count").read[Int]
        )(DockerImageSearchResult.apply _ ),
    ...
    implicit val dockerImageHistoryInfoFmt = Format(
        ...
        )(DockerImageHistoryInfo.apply _),
        Json.writes[DockerImageHistoryInfo]
    )
    
    implicit val dockerEventFmt = Format(
        (
            (__ \ "status").read[String] and
            (__ \ "id").read[ContainerId] and
            (__ \ "from").read[RepositoryTag] and
            (__ \ "time").read[Long].map(new org.joda.time.DateTime(_))
        )(DockerEvent.apply _),
        Json.writes[DockerEvent]
    )
    ...
    implicit val containerChangelogFmt = Format(
        (
            ...
            (__ \ "Kind").read[Int]
        )(ContainerChangelogRecord.apply _),
    ...
    implicit val dockerVersionFmt = Format(
        (
            ...
            (__ \ "Os").readNullable[String]
        )(DockerVersion.apply _),
    ...
    implicit val containerStateFmt = Format(
        (
            ...
            (__ \ "Ghost").read[Boolean]
        )(ContainerState.apply _),
        (
            ...
            (__ \ "Ghost").write[Boolean]
        )(unlift(ContainerState.unapply))
    )
    ...
    implicit val containerNetworkConfigFmt = Format(
        (
            ...
            (__ \ "Ports").readNullable[Map[String, DockerPortBinding]].map(_.map(_.values.toSeq))
        )(ContainerNetworkConfiguration.apply _),
        (
            ...
            (__ \ "Ports").writeNullable[Seq[DockerPortBinding]](networkConfigPortBindingWrite)
        )(unlift(ContainerNetworkConfiguration.unapply))
    )
    ...
    implicit val dockerAuthFmt = Format(
        (
          ...
          (__ \ "ServerAddress").read[String]          
        )(DockerAuth.apply _),
        (
          (__ \ "Username").write[String] and
          (__ \ "Password").write[String] and
          (__ \ "Email").write[String] and
          (__ \ "ServerAddress").write[String]          
        )(unlift(DockerAuth.unapply))
    )
    ...
    implicit val containerHostConfigFmt = Format(
        (
            ...
            (__ \ "Links").readNullable[Seq[ContainerId]]
        ...
        (
            ...
            (__ \ "Links").writeNullable[Seq[ContainerId]]
        )(unlift(ContainerHostConfiguration.unapply))
    )
    ...
    implicit val containerConfigFmt = Format(
    	(
    	    ...
    	    (__ \ "OnBuild").readNullable[Seq[String]]
    	)(ContainerConfiguration.apply _),
    	(
    	    ...
    	    (__ \ "OnBuild").writeNullable[Seq[String]]
    ...
    implicit val containerInfoFmt = Format(
        (
        	...
        	(__ \ "VolumesRW").readNullable[Map[String,Boolean]]
        )(ContainerInfo.apply _),
        (
        	...
        	(__ \ "VolumesRW").writeNullable[Map[String, Boolean]]
    ...
    implicit val containerFmt = Format(
    		(
    		    ...
    		    (__ \ "SizeRootFs").readNullable[Long]
    		)(Container.apply _),
